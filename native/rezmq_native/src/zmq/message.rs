//! This module is from https://github.com/erickt/rust-zmq/blob/master/src/message.rs.
//! Copyright (c) 2011-2014 Erick Tryzelaar
...
pub struct Message {
  pub msg: super::bindings::zmq_msg_t,
}

impl Drop for Message {
  fn drop(&mut self) {
    unsafe {
      let rc = super::bindings::zmq_msg_close(&mut self.msg);
      assert_eq!(rc, 0);
    }
  }
}
...
impl Message {
  ...
  where
    F: FnOnce(&mut super::bindings::zmq_msg_t) -> i32,
  {
    let mut msg = super::bindings::zmq_msg_t::default();
    let rc = f(&mut msg);
    if rc == -1 {
      panic!("{}", super::bindings::zmq_errno())
    }
    Message { msg }
  }
  ...
  pub fn new() -> Message {
    unsafe { Self::alloc(|msg| super::bindings::zmq_msg_init(msg)) }
  }

  /// Create a `Message` from an initialized `super::bindings::zmq_msg_t`.
  ///
  /// # Safety
  ///
  /// `msg` must be initialized.
  pub unsafe fn from_msg(msg: super::bindings::zmq_msg_t) -> Self {
    Message { msg }
  }
  ...
  pub unsafe fn with_capacity_unallocated(len: usize) -> Message {
    Self::alloc(|msg| super::bindings::zmq_msg_init_size(msg, len as size_t))
  }
  ...
  pub fn get_more(&self) -> bool {
    let rc = unsafe { super::bindings::zmq_msg_more(&self.msg) };
    rc != 0
  }
...
impl Deref for Message {
  ...
  fn deref(&self) -> &[u8] {
    ...
    unsafe {
      ...
      let data = super::bindings::zmq_msg_data(ptr);
      let len = super::bindings::zmq_msg_size(ptr) as usize;
...
impl DerefMut for Message {
  fn deref_mut(&mut self) -> &mut [u8] {
    ...
    // this message.
    unsafe {
      let data = super::bindings::zmq_msg_data(&mut self.msg);
      let len = super::bindings::zmq_msg_size(&self.msg) as usize;
...
impl From<Box<[u8]>> for Message {
  ...
  fn from(data: Box<[u8]>) -> Self {
    ...
    unsafe {
      Self::alloc(|msg| {
        super::bindings::zmq_msg_init_data(
          msg,
          raw as *mut c_void,
          len,
...
pub fn msg_ptr(msg: &mut Message) -> *mut super::bindings::zmq_msg_t {
  &mut msg.msg
}
